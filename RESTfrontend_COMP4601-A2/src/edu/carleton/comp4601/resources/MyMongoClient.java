...
public class MyMongoClient {
	...
	//A map of database collections to be stored
	HashMap<String,HashMap<String,DBCollection>> db;

	//Constructor to initialize database connection
	public MyMongoClient()  {
		
		//Try to connect to mongo client, if not throw error
		try { mc = new MongoClient("localhost", 27017);} 
		catch (UnknownHostException e) { e.printStackTrace(); }
		
		//After client refresh, declare database properties
		database = mc.getDB("COMP4601-A2");
		db = new HashMap<String,HashMap<String,DBCollection>>();
		
		//Add the collection names to the collections map
	...
	//GETTERS & SETTERS ---------------------------------------------------------------------------------------------------------------
	public synchronized MongoClient getMC() { 
		return mc; 
	}
